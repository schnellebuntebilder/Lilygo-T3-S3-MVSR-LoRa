/*
 * Receive_data: receives a 128x64 1-bit BMP and a JPEG sent by Send_data
 * over LoRa SX1276. The BMP is displayed on the SSD1306 via drawBitmap().
 * The JPEG is written to /capture.jpg on the SD card.
 *
...
// ── Reassembly constants ──────────────────────────────────────────
#define BMP_BYTES      1024     // 128*64/8
#define JPEG_MAX_SIZE  (32 * 1024)
#define PKT_DATA_SIZE  250
#define PKT_HEADER     4
#define PKT_TOTAL_MAX  140     // ceil(32768/250) safety ceiling
...
static uint8_t jpegLastPktLen  = 0;   // dataLen of packet with idx == total-1
...
// ── Show assembled bitmap fullscreen ──────────────────────────────
void showBitmap()
{
    display.clearDisplay();
...
void saveJpeg(uint32_t size)
{
    ...
    SD.remove("/capture.jpg");   // overwrite if exists
